...
class Trainer():
    ...
    def train(self, model, dataloader):
        ...
        # 分组参数
        param_groups = [
            {'params': backbone_params, 'lr': self.args.learning_rate * 0.1},  # 骨干网络使用较小学习率
            {'params': classifier_params, 'lr': self.args.learning_rate}  # 分类器使用正常学习率
        ]
        since = time.time()
        optimizer = optim.AdamW(param_groups, weight_decay=self.args.weight_decay, betas=(0.9, 0.999))
        logger.info(f'learning rate_b: {optimizer.param_groups[0]["lr"]}, learning rate_c: {optimizer.param_groups[1]["lr"]}')
        ...
        while True:
            ...
            cur_valid = self.do_test(model, dataloader, mode="val")
